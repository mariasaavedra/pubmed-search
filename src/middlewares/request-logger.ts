 ...
 */
function cleanRequestBody(body: any): any {
...
export function requestLogger(req: Request, res: Response, next: NextFunction): void {
  ...
  res.send = function(body: any): Response {
    ...
    if (res.statusCode >= 400) {
      Logger.warn('Response', `${req.method} ${req.url} returned ${res.statusCode}`, {
        requestId,
        duration: `${duration}ms`,
        response: body
      });
    } else if (res.statusCode >= 500) {
      Logger.error('Response', `${req.method} ${req.url} returned ${res.statusCode}`, {
        requestId,
        duration: `${duration}ms`,
        response: body
      });
    } else {
      Logger.debug('Response', `${req.method} ${req.url} returned ${res.statusCode}`, {
        requestId,
        duration: `${duration}ms`
      });
    }
